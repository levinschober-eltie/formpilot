...
// GET / — Submissions auflisten (read scope)
app.get("/", requireScope("read"), async (c: ApiAuthContext) => {
  ...
  if (status && STATUSES.includes(status as (typeof STATUSES)[number])) {
    conditions.push(
      eq(submissions.status, status as (typeof STATUSES)[number]),
    );
  }
  if (templateId) {
...
// GET /:id — Einzelne Submission (read scope)
app.get("/:id", requireScope("read"), async (c: ApiAuthContext) => {
  ...
  const [submission] = await db
    ...
    .from(submissions)
    .where(
      and(eq(submissions.id, id), eq(submissions.organizationId, orgId)),
    )
    .limit(1);

  if (!submission) {
    return c.json(
      { error: "Formular nicht gefunden", code: "NOT_FOUND" },
      404,
    );
  }
...
// POST / — Submission erstellen (write scope)
app.post("/", requireScope("write"), async (c: ApiAuthContext) => {
  ...
  const [created] = await db
    ...
    .values({
      ...
      filledBy: null, // API-Submissions haben keinen Profil-Bezug
      filledByName: parsed.data.filledByName ?? "API",
...
// PUT /:id — Submission aktualisieren (write scope)
app.put("/:id", requireScope("write"), async (c: ApiAuthContext) => {
  ...
  if (parsed.data.templateVersion !== undefined)
    updateData.templateVersion = parsed.data.templateVersion;
  ...
  if (parsed.data.filledByName !== undefined)
    updateData.filledByName = parsed.data.filledByName;
  ...
  if (parsed.data.projectName !== undefined)
    updateData.projectName = parsed.data.projectName;
  if (parsed.data.projectAddress !== undefined)
    updateData.projectAddress = parsed.data.projectAddress;
  if (parsed.data.signatures !== undefined)
    updateData.signatures = parsed.data.signatures;
