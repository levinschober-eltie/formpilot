...
export const CheckboxField = React.memo(({ field, value, onChange, error, ...rest }) => {
  const selected = Array.isArray(value) ? value : [];
  const toggle = (v) => onChange(selected.includes(v) ? selected.filter(x => x !== v) : [...selected, v]);
  return (
    <div role="group" aria-label={field.label} aria-describedby={rest['aria-describedby']} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {(field.options || []).map(o => (
        <label key={o.value} onClick={() => toggle(o.value)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', borderRadius: S.radius.md, border: `1.5px solid ${selected.includes(o.value) ? S.colors.primary : error ? S.colors.danger : S.colors.border}`, background: selected.includes(o.value) ? `${S.colors.primary}08` : S.colors.bgInput, cursor: 'pointer', transition: S.transition }}>
