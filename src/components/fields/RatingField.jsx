  ...
  if (field.ratingType === 'traffic') {
    ...
    return (
      <div style={{ display: 'flex', gap: '12px' }}>
        {colors.map(c => (
          <button type="button" key={c.value} onClick={() => onChange(c.value)} style={{ width: 48, height: 48, borderRadius: '50%', border: `3px solid ${current === c.value ? c.color : S.colors.border}`, background: current === c.value ? c.color : `${c.color}18`, cursor: 'pointer', transition: S.transition, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, color: current === c.value ? '#fff' : c.color, fontFamily: 'inherit' }}>{c.label}</button>
        ))}
      </div>
    );
  }
  return (
    <div style={{ display: 'flex', gap: '6px' }}>
      {Array.from({ length: max }, (_, i) => i + 1).map(star => (
        <button type="button" key={star} onClick={() => onChange(star)} style={{ fontSize: '32px', cursor: 'pointer', background: 'none', border: 'none', padding: '4px', transition: S.transition, transform: star <= current ? 'scale(1.1)' : 'scale(1)', filter: star <= current ? 'none' : 'grayscale(1) opacity(0.3)' }}>⭐</button>
      ))}
